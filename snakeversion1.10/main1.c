...
//	int snake1x = 0 , snake2x = WIDTH;			
	...
	printscreen(s1, s2, &reward,&b,&call_no,&br,p,0,0,0);
...
//		set_dir2=0;
		...
		if(c=='o')
			ch = play_one(s1, s2, &snake2, &snake2dir, &reward,p); 		
		else
			ch =play(s1, s2, &snake1, &snake2, &snake1dir, &snake2dir, &reward,p);
		if (ch==1)
			break;
		if ( c=='o')
			auto_move(s1, s2, &snake1dir, &reward, &b, &br, p, TIME);
		...
		printscreen(s1, s2, &reward,&b, &call_no, &br, p,t1,ini_time,maxtime);	
		refresh();
	}	
	while (ch!='q');
	clrtoeol();
